...
- 已关闭的事件不发送提醒；首次 baseline 仍会记录所有事件现状，避免后续开启时刷历史。
- 每个订阅项的 Star、Release、Issue、PR 分别按各自间隔判断是否需要请求 GitHub。
- 事件从关闭切换为开启时，会先把当前事件列表作为新基线记录，不补发关闭期间的历史事件。
- 同一订阅同一事件类型不并发执行。
...
- 网络超时：记录日志，下轮重试。
- JSON 解析失败：记录日志，下轮重试。
- 消息发送失败：记录日志，回滚本轮状态，下轮满足间隔后重试，避免未送达事件被永久标记为已通知。
